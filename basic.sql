...
-- CASE WHEN CONDITION THE RESULT[ WHEN SEARCH THEN RESULT ] ELSE DEFAULT END
SELECT  SALARY,
        DEPT_ID,
...
SELECT  * | [DISTICT] COLUMN_NAME | EXPR | [AS] ALIAS - 가져오고자하는 컬럼
FROM    TABLE_NAME                          - 대상 테이블
WHERE   SEARCH_CONDITION                    - 행에 대한 ㅈ한
--ORDER BY    기준컬럼 [ASC|DESC]              - 정렬할 때 사용, SELECT에 마지막에 단 한 번만 기술.
--GROUP BY    기준컬럼                         - 그룹핑(TABLE을 또 다른 작은 TABLE로 만듦)
--HAVING BY   SEARCH_CONDITION                - 그룹에 대한 조건
*/
...
[INNER] JOIN    TABLE2 USING (COLUMN) -- 필요에 따라서 USING, 다르면
